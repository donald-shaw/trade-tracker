      ...
      case NameChange | OptionExercise =>
        trace.events.filter(ev => ev.action == Action.Sell && !ev.trade_date.isBefore(event.trade_date))
          ...
          case _ =>
            ...
            (None, Some(upd_trace), event.flip(trace.net_outcome, trace.costs))
        }
    ...
    def accum(open: Map[Security, SecurityTrace], done: Traces, left: List[Event]): Traces = {
      ...
      case ev :: rest if (open.contains(ev.security)) =>
        ...
        combine(open(ev.security), ev) match {
          ...
          case (_, Some(trace), flipped_event) if trace.finalised && done.contains(ev.security) =>
            if (debug || codes.contains(ev.security.asx_id)) System.out.println(s"*** ${ev.trade_date} Finalised trace - units left: ${trace.current.count}. Outcome: ${trace.net_outcome}. There are other (finalised) traces for this security. Flipped event: $flipped_event")
            accum(open - ev.security, done.updated(ev.security, trace :: done(ev.security)), flipped_event.map(_ :: rest).getOrElse(rest))
          ...
          case (_, Some(trace), flipped_event) =>
            if (debug || codes.contains(ev.security.asx_id)) System.out.println(s"*** ${ev.trade_date} Updated (unfinalised) trace - units left now: ${trace.current.count}. Flipped event: $flipped_event")
            accum(open.updated(ev.security, trace), done, flipped_event.map(_ :: rest).getOrElse(rest))
        }
      case ev :: rest =>
